...
def cmd_scan(args):
    ...
    if not args.quiet:
        print(
            f"Scanning {len(text)} chars through {len(['all'])} detectors...\n",
            file=sys.stderr,
        )
    ...
    if args.json:
        ...
        if result.threats:
            ...
            for t in result.threats:
                print(
                    f"  [{t.severity.value.upper():8s}] {t.detector}: {t.reason} (conf: {t.confidence:.2f})"
                )
...
def cmd_test(args):
    ...
    _ = canary.insert("Secret document")
    lr = canary.check(
        "The secret document says CANARY[abcd1234abcd1234abcd1234abcd1234]"
    )
...
def main():
    ...
    scan_parser = subparsers.add_parser(
        "scan", help="Scan text through security pipeline"
    )
