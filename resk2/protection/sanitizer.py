...
_DEFAULT_RULES = [
    ...
    (r'<[^>]*(?:display\s*:\s*none|visibility\s*:\s*hidden|opacity\s*:\s*0|color\s*:\s*(?:white|transparent|#[fF]{3,6})\b)[^>]*>[\s\S]*?</[^>]+>', ''),
    ...
    (r'data\s*:\s*(?:text|image|application)/[^;]+;base64,[A-Za-z0-9+/]+={0,2}', '[DATA_URI_REMOVED]'),
...
class InputSanitizer:
    ...
    def __init__(self, rules: list[tuple[str, str]] | None = None):
        ...
        for pattern, replacement in rule_set:
            self._rules.append((re.compile(pattern, re.IGNORECASE | re.DOTALL), replacement))
            ...
            if new_result != result:
                ...
                for item in removed[:10]:  # Limit tracking
                    self._last_removals.append({
                        ...
                        "removed": item[:100] if isinstance(item, str) else str(item)[:100],
                    })
                result = new_result
