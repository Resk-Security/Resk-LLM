...
class BypassDetector(BaseDetector):
        ...
        if cfg_path.exists():
            ...
            for entry in section.get("stealth") or []:
                self._compiled_stealth.append(
                    (
                        re.compile(
                            entry["pattern"], re.IGNORECASE | re.MULTILINE | re.DOTALL
                        ),
                        entry,
                    )
                )
    ...
    def detect(self, text: str, **kwargs: Any) -> DetectionResult:
        ...
        if total_encoded > 0:
            thresh = (
                (_load_config() or {}).get("thresholds", {}).get("bypass_detection", {})
            )
            confidence = min(
                0.95,
                thresh.get("encoded_base_confidence", 0.7)
                + total_encoded * thresh.get("encoded_increment", 0.1),
            )
            ...
            confidence = min(
                0.9,
                thresh.get("jailbreak_base_confidence", 0.5)
                + total_jb * thresh.get("jailbreak_increment", 0.1),
            )
            ...
            confidence = max(
                0.3,
                thresh.get("stealth_base_confidence", 0.3)
                + total_jb * thresh.get("jailbreak_increment", 0.15)
                + total_stealth * thresh.get("stealth_per_stealth", 0.15),
            )
